...
public class MatchTest {
    ...
    public void testSoulMatch() {
        ...
        log.info("--- 匹配结果指标报告 ---");
        if (results.isEmpty()) {
            log.error("[异常] 未生成任何匹配结果。原因可能：1.用户answers字段为空 2.池中用户少于2人");
        } else {
            for (MatchResult mr : results) {
                log.info("甲方: " + mr.getUserNameA() + " | 乙方: " + mr.getUserNameB());
                log.info("契合度得分: " + mr.getScore() + "%");
                log.info("匹配文案: " + mr.getDescription());
                log.info("联系方式同步: A-" + mr.getNumberA() + " | B-" + mr.getNumberB());
            }
        }
        log.info("-----------------------");
    }
